...
readonly COMMAND_FILE='../data//Bay_Area/eval//001/Incremental_commands.txt'
readonly SERVER_RESPONSE_OUTPUT='./client_output/001/serverResponses.txt'

readonly ADDITIONAL_PSL_OPTIONS='-D runtime.db.intids=True -D runtimestats.collect=true'
readonly ADDITIONAL_CLIENT_OPTIONS='-D runtime.log.level=TRACE'
...
function waitForServer() {
  ...
  while [ $i -le $MAX_CONNECTION_ATTEMPTS ]; do
    if [ -f "/tmp/onlinePSLServer/onlinePSLServer.lock" ]; then
      break
    fi
...
function runClientCommand() {
   ...
   java -Xmx8G -Xms8G -jar "${JAR_PATH}" -onlineServerOutput ${SERVER_RESPONSE_OUTPUT} ${ADDITIONAL_CLIENT_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@" < ${COMMAND_FILE}
   if [[ "$?" -ne 0 ]]; then
